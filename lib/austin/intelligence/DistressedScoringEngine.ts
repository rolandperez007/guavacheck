...
export class DistressedScoringEngine {
  static score(property: {
    ...
    const conditionMultiplier =
      property.condition === "poor" ? 1.3 : property.condition === "fair" ? 1.1 : 1;

    const locationRisk = property.location?.includes("luxury") ? 0.8 : 1;

    const finalScore = Math.min(100, priceGapScore * conditionMultiplier * locationRisk);
    ...
    return {
      ...
      grade: finalScore > 75 ? "A+" : finalScore > 50 ? "A" : finalScore > 30 ? "B" : "C",
