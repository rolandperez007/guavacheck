...
class GovernmentStage:
    ...
    async def execute(
        ...
        country = property_data.get("country", "NG")

        documents = getattr(context, "documents", [])

        compliance = self.service.verify_document_requirements(country, documents)

        sources = self.service.find_verification_sources("verify_land_title")
        ...
        context.evidence.append(
            {"type": "government", "data": context.government_result}
        )
