...
export default function PredictiveIntelligence() {
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-950/90 p-6">
      <div>
        <p className="text-sm uppercase tracking-[0.35em] text-slate-500">Predictive intelligence</p>
        <h3 className="mt-2 text-xl font-semibold text-white">Austin forecasts and recommendations</h3>
      </div>

      <div className="mt-5 grid gap-3 md:grid-cols-2">
        {forecasts.map((forecast) => (
          <div key={forecast.label} className="rounded-xl border border-slate-800 bg-slate-900/80 p-4 text-sm text-slate-300">
